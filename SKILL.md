...
ZCode 按优先级发现技能：`<项目>/.zcode/skills/` > `<项目>/.agents/skills/` > `~/.zcode/skills/` > `~/.agents/skills/`。
**同名技能以路径为身份**，优先级高的那份生效，低优先级的不会报错、只会被遮蔽 ——
所以同一台机器上不要留两份想同时生效的副本。

- 通用方法论技能（本技能即此类）装 `~/.agents/skills/<name>/`：所有项目共用。
- 只对某个仓库有意义的技能装该仓库的 `.agents/skills/<name>/`，随仓库提交、团队共享。
- 如果宿主是别的 Agent 平台（其他 Agent 宿主 等），它的技能目录与本技能目录是**两份物理副本**，
  改完要同步过去，否则两边行为会分叉。同步时以逻辑内容一致为准，不要只改一边。
